...
public class ZoomIn implements Ability {
    ...
    public void execute(Personaje user, Personaje target) {
        ...
        if (user.accuracy + 15 <= 100) {
            user.accuracy += 15;
            System.out.println(">> " + user.getNombre() + " stabilizes their breathing and adjusts the optics.");
            System.out.println("   [BUFF] Accuracy increased to " + user.accuracy + "%.");
        } else {
            System.out.println(">> " + user.getNombre() + " attempts to zoom in, but they already have a perfect sight picture.");
